...
def sel_sort(numbers:list, reverse:bool = False) -> list:
    ...
    result = []     # 빈 리스트 생성
    while numbers_copy:  # numbers의 원소가 있는 동안에   # False: 0, "", [], {}
        # print('numbers = ', numbers_copy)
        # print('result = ', result)
        if reverse:     # True: 내림차순 정렬
            _, found = find_max(numbers_copy)   # 최대값 찾음
        else:       # False: 오름차순 정렬
            _, found = find_min(numbers_copy)  # 최소값을 찾음   # min_id는 안씀
        result.append(found)  # 결과 리스트에 추가
        numbers_copy.remove(found)  # 카피본에서 최대/최소값 삭제
    return result
...
def sel_sort2(numbers: list, reverse:bool = False) -> None:
    ...
    for i in range(0, length - 1):
        ...
        for j in range(i + 1, length):
            # j: 최대값/최소값을 찾기 위해서 비교할 원소들의 인덱스
