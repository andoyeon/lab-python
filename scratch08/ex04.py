...
모델 y = ax + b 에서, 기울기(slope) a와 y절편 b를 찾는 문제
...
def linear_gradient(x, y, theta):
    ...
    gradient = [error * x, error]   # 절편의 gradient(error)와 반대 방향으로 움직임
    return gradient
...
if __name__ == '__main__':
    ...
    for epoch in range(200):   # 임의의 회수(epoch)만큼 반복
        ...
        for x, y in dataset:
            ...
            theta = gradient_step(theta, gradient, -step)   # 에러를 최소화(-step)
        if (epoch + 1) % 10 == 0:
            print(f'{epoch}: {theta}')
