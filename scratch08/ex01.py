...
(예) 모델의 오류(error)를 "최소화", likelihood(우도, 가능도)를 "최대화"
...
if __name__ == '__main__':
    ...
    # x**2 그래프에서 x=-1에서의 접선을 그리기 위해
    # 직선의 방정식 y = ax + b 에서 기울기 a와 y절편 b를 찾아야 함.
    ...
    # 모든 점에서
    # 실제 기울기(f_derivation)와 기울기 근사값(difference_quotient)
    ...
    for _ in range(5):
        # x = init_x에서의 접선의 기울기
        gradient = difference_quotient(f, init_x, h=0.01)   # f'(x)=2x
        # 접선을 그래프로 그리기 위해서
    ...
    random.seed(1128)
    init_x = random.randint(-10, 10)    # 시작값
    print(f'init_x = {init_x}')
    tolerance = 0.0000001
    # 두 x좌표 사이의 거리가 tolerance 이하이면 반복문 종료하도록
    count = 0
    while True: # 반복
        count += 1
        # x좌표에서의 접선의 기울기를 계산
