...
for _ in range(trials): # 10,000번 반복
    ...
    if random_coin == 'H':
        ...
        tails += 1
p_h = heads / trials    # 앞면이 나올 확률 = 앞면 횟수 / 전체 횟수
p_t = tails / trials    # 뒷면이 나올 확률
print('P(H):', p_h, '|', 'P(T):', p_t)
...
def experiment(type, n, t):
    ...
    for _ in range(t):  # 실험 횟수만큼 반복
        ...
        # Counter 클래스는 tuple의 개수는 셀수 있지만,
        # list의 개수는 셀 수 없음!
        # Counter은 정렬 가능한 타입만 사용할 수 있음
...
num_of_cases = 0
for ev, cnt in coin_event_counts.items():
    if how_many_head(ev) == 1 or how_many_head(ev) == 2:
        num_of_cases += cnt
...
#   (2, 6), (3, 5), (4, 4), (5, 3), (6, 2)
# 주사위 2개를 던졌을 때, 적어도 하나가 짝수가 나올 확률 27/36
# = 1 - 두 수가 모두 홀수인 확률
