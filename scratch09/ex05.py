...
# DataFrame.shape: (row 갯수, column 갯수)
...
print(gapminder.iloc[0:5])  # gapminder.head()
print(gapminder.iloc[nrows - 5:nrows])  # gapminder.tail()  # 인덱스 a:b는 a <= x < b를 의미함.
...
# row_indices = [0, 99, 999]
# print(gapminder.iloc[row_indices])
print(gapminder.iloc[[0, 99, 999]])

# DataFrame.iloc[row index, column index]
# DataFrame.loc[row label, column label]
# loc에서 범위 연산자(:)이 사용되면 이름(label)로 취급하기 때문에 양쪽 숫자 모두 포함
# iloc에서 범위 연산자(:)가 사용되면 인덱스로 취급하기 때문에 뒤쪽 숫자 미포함
