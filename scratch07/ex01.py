...
def normal_approximation_to_binomial(n, p):
    """ 이항분포(n, p)를 정규 분포로 근사했을 때 평균, 표준 편차 """
    mu = n * p
...
def normal_two_sided_bounds(prob, mu=0.0, sigma=1.0):
    ...
    # P(X > a) = tail_prob을 만족하는 하한 a을 찾으면 됨
    upper_bound = normal_lower_bound(tail_prob, mu, sigma)
    # P(X < b) = prob + tail_prob을 만족하는 상한 b를 찾으면 됨
    # upper_bound = normal_upper_bound(prob + tail_prob, mu, sigma)

    # 찾으려는 하한(lower bound)은
    # 확률 tail_prob 이하를 갖는 상한을 찾아면 됨
...
if __name__ == '__main__':
    # 동전을 던졌을 때, 앞면이 나올 확률: 1/2(=0.5) testing(검정)
    # 동전을 1,000번 던지는 실험 - 이항 분포
    # 앞면이 나오는 기대값 - 정규 분포(np, sqrt(np(a-p))
    ...
    print('p = 0.5 가정: mu_0 =', mu, 'sigma_0 =', sigma)

    # 유의 수준 5%:
    # H0이 참이지만 기각을 하는 오류 5%를 감수
    ...
    # 동전 앞면이 나올 확률을 0.55라고 가정했을 때, 평균/표준 편차
    mu_1, sigma_1 = normal_approximation_to_binomial(1000, 0.55)
    print('p = 0.5 가정: mu_1 =', mu_1, 'sigma_1 =', sigma_1)
    # p=0.5라는 가정에서 95% 구간이, p=0.55라는 가정에서 나올 확률
    beta = normal_probability_between(low, high, mu_1, sigma_1) # (겹치는 부분)
    # p=0.55라는 가정이 맞았다고 할 오류
    print('beta =', beta)   # 0.11
    power = 1 - beta
    # p=0.55라는 가정이 틀렸다고 검증할 수 있는 능력
    print('power =', power) # 0.8865
    # => 동전 앞면이 나올 확률은 0.55가 아니다
    ...
    p_bar = 525/1000
    mu = p_bar  # 모집단의 평균을 실험값의 평균(표본 평균)으로 대체
    ...
    # 동전을 1000번 던졌을 때, 앞면이 540번 발생
    p_bar = 540 / 1000  # 표본 평균
    mu = p_bar  # 모집단 평균을 표본 평균으로 가정
    sigma = math.sqrt(p_bar * (1 - p_bar) / 1000)
    bounds = normal_two_sided_bounds(0.95, mu, sigma)
    ...
    # p-value > 0.05
    # -> A와 B가 차이가 우연히 발생할 확률은 유의 수준보다 높다.
